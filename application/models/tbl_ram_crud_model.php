...
class Tbl_ram_crud_model extends CI_Model
{
	...
	public function get_rams( $id_cpu = FALSE )
	{
		if ( $id_cpu === FALSE ) {
	...
	{
		$this->db->from('tbl_ram');
		$this->db->join('tbl_empleados', 'tbl_ram.id_empleado=tbl_empleados.id_empleado');
		$this->db->where('tbl_ram.id_empleado',$id_empleado);
		$res=$this->db->get();
		return $res->result(); 
	}
	...
	public function agregar_carpetas($carpetas_geaco06, $carpeta_imagenes,  $capacidad_correo, $otros_servicios, $id_empleado)
	{
		...
		$this->db->where('tbl_ram.id_empleado',$id_empleado);
		$nuevo = $this->db->insert('tbl_ram', $data);
