		...
		<table class="table table-bordered">    
				...
				<?php foreach ($cargar_users as $fila) :?> <!--//es tipo un contador que entra a un arreglo y me trae todos los registros hasta que terminen-->
					...
					<td><a href="" data-toggle="modal" data-target="#miModal<?php echo $fila->id_user; ?>">Editar</a>
							      	...
							      	<div class="modal-body">
							        	<form id="ModalEditarUser" role="form" action="<?php echo site_url('users/actualizar');?>" method="post">     
												...
												<div class="form-group">        
													...
													<?php
													if ($fila->id_status == '1') { ?>
													 	<input type="radio" name="id_status" value="1" checked> Activar
													 	<input type="radio" name="id_status" value="0"> Desactivar
													<?php } else { ?>
														<input type="radio" name="id_status" value="1"> Activar
													 	<input type="radio" name="id_status" value="0" checked> Desactivar
													<?php } ?>	  
				      						...
				      						<div class="modal-footer">
				      							<a type="btn" class="btn btn-danger" href="" data-toggle="modal" data-target="#miModalDeleteUser<?php echo $fila->id_user; ?>">Eliminar Usuario</a>
						...
						<!-- Modal -->
                    <div id="miModalDeleteUser<?php echo $fila->id_user; ?>" class="modal fade" role="dialog">
                        <div id="moduloseg" class="modal-dialog">
                        <!-- Modal content-->
                          <div class="modal-content">
                              <div class="modal-header">
                                <button type="button" class="close" data-dismiss="modal">&times;</button>
                                <h3 class="modal-title">¿Desea eliminar el usuario de forma permanente?</h3>
                              </div>
                              <div class="modal-body">
                                <form id="ModalEliminarPresentacion" role="form" action="<?php echo site_url('users/eliminar');?>" method="post">
                                  <input type="hidden" class="form-control" id="id_user" name="id_user" value="<?php echo $fila->id_user; ?>">
                                  <div class="form-group">    
                                    <label for="nombre">NOMBRE: <?php echo $fila->nombre; ?></label>       
                                  </div>
                                  <div class="modal-footer">
